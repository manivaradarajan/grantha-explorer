"""Tests for the Śaṅkara-edition ingestion work.

Covers four new behaviors introduced alongside ingesting the Śaṅkara bhāṣyas:

- ``--exclude-editions`` (importer): fnmatch exclusion of editions before
  grouping, including the ``< 2``-edition skip interaction.
- ``--grantha-id`` (importer): exact grantha filter for co-located granthas
...
class TestExcludeEditions(unittest.TestCase):
    ...
    def _make_source(self, root: Path, rules_text: str) -> None:
    ...
    def test_exclude_keeps_two_edition_grantha(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            envelope = json_load(lib / "upanishads" / "isavasya" / "envelope.json")
...
class TestGranthaIdFilter(unittest.TestCase):
    ...
    def test_grantha_id_exact_match_imports_only_upanishad(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            envelope = json_load(lib / "upanishads" / "mandukya" / "envelope.json")
            editions = [e["edition_id"] for e in envelope["editions"]]
            self.assertEqual(
                editions,
                [
